...
async fn rss_proxy(q: web::Query<ProxyQuery>) -> Result<String, LoggingError> {
    ...
    let proxied = q.proxied.as_str().try_into()
        .with_context(|| format!("Expected query parameter 'proxied' with value '{}' to be url.", q.proxied))?;
    let feed = download_feed(&RssDeserializer {}, &proxied).await
        .with_context(|| format!("Failed to download the proxied feed {}", proxied))?;
    
    let proxied2 = q.proxied2.as_str().try_into()
        .with_context(|| format!("Expected query parameter 'proxied2' with value '{}' to be url.", q.proxied))?;
    let feed2 = download_feed(&RssDeserializer {}, &proxied2).await
        .with_context(|| format!("Failed to download the proxied2 feed {}", proxied2))?;

    let combined = merge_feeds("feeds/rss-proxy".into(), "https://feedragon.privacy.qvarford.net/feeds/rss-proxy".try_into().unwrap(), vec![feed, feed2]);
